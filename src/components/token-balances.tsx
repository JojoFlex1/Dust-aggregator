...
import { DustAggregatorClient } from "@/lib/soroban-client";
...
export type DustBalance = {
  ...
  amount: string;
  value: number;
  asset?: string; // Added for Soroban compatibility
  userAddress?: string; // Added for Soroban compatibility
};
...
export default function DustAggregator() {
  ...
  const userSecret = process.env.NEXT_PUBLIC_USER_SECRET || ""; // Added for Soroban client
  ...
  const connectWallet = async (chain: string, walletInfo: any) => {
    try {
      ...
      } else if (chain === "stellar" && typeof window !== "undefined" && window.freighter) {
    ...
    try {
      ...
      if (wallet.chain === "ethereum") {
        ...
        newBalances.push({
          ...
          chain: "ethereum",
          token: "ETH",
          asset: "ETH", // Added for Soroban compatibility
          userAddress: wallet.address, // Added for Soroban compatibility
          amount: formattedBalance,
          value: parseFloat(formattedBalance) * 2000, // Fixed ETH price
        });
        ...
        newBalances.push({
          ...
          chain: "solana",
          token: "SOL",
          asset: "SOL", // Added for Soroban compatibility
          userAddress: wallet.address, // Added for Soroban compatibility
          amount: formattedBalance.toString(),
          value: formattedBalance * 180, // Fixed SOL price
        });
        ...
        newBalances.push({
          ...
          chain: "polygon",
          token: "MATIC",
          asset: "MATIC", // Added for Soroban compatibility
          userAddress: wallet.address, // Added for Soroban compatibility
          amount: formattedBalance,
          value: parseFloat(formattedBalance) * 1, // Fixed MATIC price
        });
      } else if (wallet.chain === "stellar") {
        // Import Server dynamically to avoid SSR issues
        ...
        try {
          ...
          account.balances.forEach((balance: any) => {
            newBalances.push({
              ...
              asset: balance.asset_code || "XLM", // Added for Soroban compatibility
              userAddress: wallet.address, // Added for Soroban compatibility
              amount: balance.balance,
              value: parseFloat(balance.balance) * 0.1, // Fixed XLM price
            });
          });
        } catch (error) {
  ...
  const processBatch = async () => {
    try {
      ...
      const sorobanClient = new DustAggregatorClient({
        ...
        userSecret: userSecret, // Added required parameter
      });
      ...
      for (let i = 0; i < dustBalances.length; i++) {
        try {
          ...
          const depositResult = await sorobanClient.autoDeposit({
            ...
            chain: balance.chain,
            asset: balance.asset,
            amount: BigInt(Math.round(balance.value * 100)), // Convert to bigint with 2 decimal places
          });
          
          console.log(`Deposited ${balance.token}:`, depositResult);
      ...
      try {
        const batchResult = await sorobanClient.batchProcess({
          userAddress: stellarWallet.address,
          contractAddress: sorobanContractAddress,
          networkPassphrase: stellarNetworkPassphrase,
          rpcUrl: stellarRpcUrl,
        ...
        console.log("Batch process result:", batchResult);
  ...
  const transferToStellar = async () => {
    try {
      ...
      // Soroban transfer
      const transferResult = await sorobanClient.autoDeposit({
        userAddress: stellarWallet.address,
        contractAddress: sorobanContractAddress,
        networkPassphrase: stellarNetworkPassphrase,
        rpcUrl: stellarRpcUrl,
        ...
        chain: "stellar",
        asset: targetAsset,
        amount: BigInt(Math.round(batchedDust.totalValue * 100)), // Assuming contract uses 2 decimals
      });

      console.log("Soroban transfer result:", transferResult);
  ...
  const getChainColor = (chain: string) => {
    ...
    return colors[chain] || "bg-blue-600";
  };
